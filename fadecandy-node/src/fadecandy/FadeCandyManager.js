...
export default class FadeCandyManager extends EventEmitter {
  constructor({ logger, mapping = BOOKSHELF_MAP, pixelCount } = {}) {
    ...
    this.shelfState = listShelves(mapping).reduce((acc, { columnIndex, shelfIndex }) => {
      acc[`${columnIndex}:${shelfIndex}`] = { r: 0, g: 0, b: 0 };
      return acc;
    }, {});
  ...
  listShelves() {
    return listShelves(this.mapping).map((entry) => ({
      ...entry,
      color: this.shelfState[`${entry.columnIndex}:${entry.shelfIndex}`],
    }));
  }

  clear({ skipSend = false } = {}) {
    this.frame.fill(0);
    Object.keys(this.shelfState).forEach((key) => (this.shelfState[key] = { r: 0, g: 0, b: 0 }));
    if (!skipSend) this.pushFrame();
  }

  setShelfColor(columnIndex, shelfIndex, color, { flush = true } = {}) {
    ...
    this.shelfState[`${columnIndex}:${shelfIndex}`] = rgb;
    if (flush) this.pushFrame();
  }

  setAllShelves(color, { flush = true } = {}) {
    const rgb = normalizeColor(color);
    listShelves(this.mapping).forEach(({ columnIndex, shelfIndex }) => {
      this.setShelfColor(columnIndex, shelfIndex, rgb, { flush: false });
    });
    if (flush) this.pushFrame();
  }
    ...
    return {
      ...
      sparkle: ({ density = 0.08, base = { r: 5, g: 5, b: 5 } } = {}) => {
        return withFrameRate(() => {
          ...
          this.frame.fill(0);
          this.setAllShelves(base, { flush: false });
